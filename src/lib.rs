...
// TO DO : Add function to get next date and previous date in the Republican calendar, and to get the difference between two dates in the Republican calendar
...
pub fn display_date(republican_date: RepublicanDate) -> String {
    let day_name = republican_date.get_day_name();
    let month_name = republican_date.get_month().to_string();
    let year = republican_date.get_year();

    return format!("{} de {} de l'an {}", day_name, month_name, year);
}
...
mod tests {
    ...
    fn test_date_calculator_romme() {
        ...
        assert_eq!(
            republican_date.get_date_string(),
            "Septidi de Ventôse de l'an 234"
        );
        ...
        assert_eq!(
            republican_date.get_date_string(),
            "Primidi de Vendémiaire de l'an 1"
        );
        ...
        assert_eq!(
            republican_date.get_date_string(),
            "4 jour complémentaire de l'an 7"
        );

        let date = Date::from_calendar_date(1796, time::Month::September, 21).unwrap(); // The second anniversary of the start date, which is the 6 th complementary day of the year 4, which is a leap year
        let republican_date = date_calculator_romme(date);
        assert_eq!(
            republican_date.get_date_string(),
            "6 jour complémentaire de l'an 4"
        );
    }
}
